exports.validateSignupInput = function(objRepo) {
  ...
  return (req, res, next) => {
    ...
    if (!fullname || !username || !email || !password) {
      return next(new AppError('All fields is required.', httpStatusCodes.BAD_REQUEST));
    }
...
exports.validateRefreshTokenInput = function(objRepo) {
  ...
  return (req, res, next) => {
    ...
    if (!decodedToken) {
      return next(new AppError('Refresh token is invalid.', httpStatusCodes.UNAUTHORIZED));
    }
